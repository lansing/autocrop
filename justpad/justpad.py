...
def crop(image, fheight=500, fwidth=500,
    ...
    prepad_height = int(height * (1.0 - facepad))
    prepad_width = int(width * (1.0 - facepad))

    scaled_prepad_height = int(prepad_height * (fheight / height))
    scaled_prepad_width = int(prepad_width * (fwidth / width))

    image = cv2.resize(image, (scaled_prepad_width, scaled_prepad_height),
